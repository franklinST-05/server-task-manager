...
export default function verifyEmailTemplate({ token }: VerifyEmailTemplateProps) {
    ...
    <body style="margin: 0px; padding: 0px;box-sizing: border-box;">
      <style>
        * { margin: 0px; padding: 0px; box-sizing: border-box; transition: all 0.3s linear; }
        ...
        @media screen and (max-width: 500px) { .auto-layer { flex-direction: column; text-align: center; } }
      </style>
      <main style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; padding:20px;">
        <h1 style="font-size: 24px; display: block;">Hello welcome to TaskManager</h1>
        <p style="font-size: 14px; color: #57575c;">We are close to creating your account, click the button below to complete your registration.</p>
        <div class="auto-layer" style="padding: 20px 0px; gap: 10px;">
          <a href="${path}" style="display: block; width: max-content; padding: 10px 12px; font-size: 14px; font-weight: 600; background-color: #96f; color: #fff; cursor: pointer;">CONFIRM</a>
          <div class="auto-layer" style="width: 100%; max-width: 300px; gap: 8px;">
            ...
            <p style="font-size: 12px; color: #57575c;">If you were not the one who requested access, please ignore this email and do not share it.</p>
          </div>
        </div>
      </main>
    </body>
